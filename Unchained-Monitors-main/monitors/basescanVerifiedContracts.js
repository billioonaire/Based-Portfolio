...
async function scrapeBasescan() {
    try {
        const response = await axios.get('https://basescan.org/contractsVerified');
        ...
        currentAddresses.forEach(async(address) => {
            if (!knownAddresses.has(address)) {
                ...
                if (!isFirstRun) {
                    ...
                    console.log("New BASE Verified Contract: ", address);
...
function startVerifiedContractMonitor(discordClient) {
    ...
    console.log("Starting BaseScan Verified Contract Monitor")
    setInterval(scrapeBasescan, 5000);
}
